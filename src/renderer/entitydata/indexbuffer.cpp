...
std::tuple<uint32_t, uint32_t> registerIndices(const std::vector<uint32_t>& indices)
{
	...
	r_globalIndexVector
		.insert(r_globalIndexVector.end(), indices.begin(), indices.end());
...
bool createIndexBuffer()
{
	...
	const VkDeviceSize bufferSize =
		sizeof(r_globalIndexVector[0]) * r_globalIndexVector.size();
	VkBuffer stagingBuffer;
	VkDeviceMemory stagingBufferMemory;
	if (!createBuffer(
		    stagingBuffer,
		    stagingBufferMemory,
		    "Main index staging buffer",
		    bufferSize,
	...
	setHandleName(
		reinterpret_cast<uint64_t>(r_indexBuffer),
		VK_OBJECT_TYPE_BUFFER,
		"entity index buffer"
	);
	setHandleName(
		reinterpret_cast<uint64_t>(r_indexBufferMemory),
		VK_OBJECT_TYPE_DEVICE_MEMORY,
		"entity index buffer memory"
	);
