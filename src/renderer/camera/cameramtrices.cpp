...
namespace clz::renderer::camera
{
	...
	{
		view = makeViewMatrix(Position[activeCamera],
			Position[activeCamera] + localFront[activeCamera],
			WorldUp);;
		return view.value();
	}
	math::mat4 getProjectionMatrix()
	{
		...
		{
			projection = math::makePerspectiveMatrix(100.0f, 1.0f,
				static_cast<float>(r_swapchainContext.extent.width) / r_swapchainContext.extent.height,
