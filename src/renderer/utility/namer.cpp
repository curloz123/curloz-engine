...
void setHandleName(const uint64_t handle, const VkObjectType objectType, const char* name)
{
	...
	// NOTE: Since this function only passes the device, we need access to the instance
	// pointer. Swap 'r_instanceContext.instance' below with whatever your global
	// instance variable is named.
	...
	if (pfnSetDebugUtilsObjectNameEXT)
	{
		...
		// r_deviceContext.device is already being used successfully in your other
		// files
		pfnSetDebugUtilsObjectNameEXT(
