...
namespace clz::renderer
{
	...
	)
	{
		...
		{
			clz::log::error(
				"unable to load file: " + std::string(vertexShaderLocation)
			);
		}
		if (!frag_file)
		{
			clz::log::error(
				"unable to load file: " + std::string(fragmentShaderLocation)
			);
		}
		...
		if (vkCreateShaderModule(
			    ...
			    &fragInfo,
			    nullptr,
			    &rPipelineContext.fragmentShader
		    ) != VK_SUCCESS)
		{
			clz::log::error(
		...
		clz::log::info(
			"created shader modules: " + std::string(vertexShaderLocation) + " and " +
			std::string(fragmentShaderLocation)
		);
		return true;
	}
	...
	createShaderStageInfo(VkShaderModule& shaderModule, ShaderStage stage)
	{
	...
	createDynamicStates(const std::vector<VkDynamicState>& dynamicStates)
	{
	...
	createInputAssemblyState(VkPrimitiveTopology topology)
	{
	...
	createViewportState(const VkViewport& viewport, const VkRect2D& scissor)
	{
	...
	createDepthStencilState(const bool depthEnable, const bool stencilEnable)
	{
	...
	VkPipelineRenderingCreateInfo createPipelineRenderingInfo(
	...
	{
		clz::CLZ_ASSERT(
			attachmentFormats.size() == attachmentCount,
			"attachmentFormats size and attachmentCount must be the same"
		);
