...
namespace clz::renderer
{
	...
	bool createUniformBuffers()
	{
		...
		if (!createBuffer(uboMemory.buffer, uboMemory.memory, bufferName, totalUBOMemory, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
				  VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT))
		{
			clz::log::error("Could not create uniform buffers!");
			return false;
		}
		vkMapMemory(r_deviceContext.device, uboMemory.memory, 0, totalUBOMemory, 0, &uboMemory.mappedMemory);

		setHandleName(reinterpret_cast<uint64_t>(uboMemory.buffer), VK_OBJECT_TYPE_BUFFER, bufferName.c_str());
		setHandleName(reinterpret_cast<uint64_t>(uboMemory.memory), VK_OBJECT_TYPE_DEVICE_MEMORY, bufferMemoryName.c_str());
		...
		for (size_t i = 0; i < pDescriptorSetLayouts.size(); ++i)
		{
			...
			if (vkCreateDescriptorSetLayout(r_deviceContext.device, &layoutInfo, nullptr, pDescriptorSetLayouts[i]) != VK_SUCCESS)
			{
				clz::log::error("vulkan Could not initialize descriptor set layout: " + descriptorSetLayoutNames[i]);
				return false;
			}

			setHandleName(reinterpret_cast<uint64_t>(*(pDescriptorSetLayouts[i])), VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT,
