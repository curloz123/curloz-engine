...
bool createDescriptors()
{
	...
	// --- Allocate camera UBO descriptor sets, one per frame-in-flight ---
	{
		...
		for (uint32_t i = 0; i < r_FRAMES_IN_FLIGHT; ++i)
		{
			setHandleName(
				...
				("camera descriptor set [frame " + std::to_string(i) + "]")
					.c_str()
			);
		}
	}
	...
	// --- Allocate sampler descriptor sets, one per frame-in-flight ---
	{
		...
		{
			setHandleName(
				reinterpret_cast<uint64_t>(samplerDescriptorSets[i]),
				VK_OBJECT_TYPE_DESCRIPTOR_SET,
	...
	clz::log::info(
		"Created and wrote camera descriptor sets; sampler sets allocated, awaiting "
		"texture data"
	);
	return true;
}
