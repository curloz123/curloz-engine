...
void drawAllModels(const VkCommandBuffer commandBuffer)
{
	const auto& model_entities =
		clz::ecs::getEntitiesWithComponent<ModelComponent>();
	...
	for (const auto& primitive : mesh.primitives)
	{
		...
		fastgltf::iterateAccessorWithIndex<fastgltf::math::fvec3>(
			...
			[&](const fastgltf::math::fvec3& pos, const std::size_t posIndex) {
				positions[posIndex] = {pos.x(), pos.y(), pos.z()};
			}
		);
		...
		{
			primitiveData.uvs =
				std::vector(positions.size(), math::vec2(0.0f, 0.0f));
...
)
{
	    ...
	    ))
	{
		const std::filesystem::path texturePath =
			...
			std::get<fastgltf::sources::URI>(primitiveData.baseTexture.value())
				.uri.c_str();
