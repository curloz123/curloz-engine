...
namespace clz::renderer
{
	...
	loadModel(const std::filesystem::path& filePath)
	{
		...
		{
			const auto error =
				getErrorMessage(assetResult.error());
	...
	void drawAllModels(VkCommandBuffer& commandBuffer)
	{
		const auto& model_entities =
			clz::ecs::getEntitiesWithComponent<ecs::ModelComponent>();

		for (const auto model_entity: model_entities)
		{
			const auto& modelId =
				ecs::getComponent<ecs::ModelComponent>(model_entity).modelId;
	...
	{
		CLZ_ASSERT(modelId < ModelLUT.size()
			&& modelId != NULL_MODEL,
	...
	{
		CLZ_ASSERT(modelId < ModelLUT.size() && modelId != NULL_MODEL,
...
namespace clz::renderer
{
	void Model::draw(const math::mat4& modelMatrix,
		VkCommandBuffer commandBuffer)
	{
	...
	void Model::drawNode(const auto& node,
		...
		VkCommandBuffer commandBuffer)
	{
		...
		for (const auto& primitive : meshes[node.meshIndex.value()].value().primitives)
		{
			...
			ModelDataPC pc = {
				...
				.textureID = primitive.baseTextureId.has_value() ?
					primitive.baseTextureId.value() :
					r_NULL_TEXTURE
			};
	...
	Node Model::loadNode(const fastgltf::Node& node, const fastgltf::Asset& asset,
		const ModelId registeredId)
	{
		...
		else if (const auto* trs = std::get_if<fastgltf::TRS>(&node.transform))
		{
			const math::vec3 translation(
				...
				trs->translation[1],
				trs->translation[2]);
			// fastgltf stores x,y,z,w. Our's is wxyz
			const math::quat rotation(
				trs->rotation[3],
				trs->rotation[0],
				trs->rotation[1],
			...
			ourNode.localTransform =
				math::getModelMatrix(rotation,
					translation,
					scale);
		}
	...
	Mesh Model::loadMesh(const fastgltf::Mesh& mesh,
		...
		const ModelId registeredId)
	{
		...
		for (const auto& primitive : mesh.primitives)
		{
			...
			fastgltf::iterateAccessorWithIndex<fastgltf::math::fvec3>(
				asset,
				positionAccessor,
				[&](const fastgltf::math::fvec3& pos,
					const std::size_t posIndex) {
					positions[posIndex] = {
						pos.x(), pos.y(), pos.z()
					};
				});
			primitiveData.vertices =
				std::vector(positions.begin(), positions.end());
			...
			primitiveData.indices =
				std::vector(indices.begin(), indices.end());
			...
			// Submitting data to buffers
			ourMesh.primitives.emplace_back(
				registerPrimitive(primitiveData,
					asset,
					registeredId));
		}
	...
	Primitive Model::registerPrimitive(const PrimitiveData& primitiveData,
		const fastgltf::Asset& asset, const ModelId registeredId)
	{
		...
		auto getTextureIdIfExistsInCache = [&](
			std::unordered_map<size_t, TextureID>& textureCache,
			const size_t textureIndex)
		{
			if (const auto& it = textureCache.find(textureIndex);
				it != textureCache.end())
			{
			...
			primitiveData.baseTexture.value()))
		{
			const std::filesystem::path texturePath =
				...
				std::get<fastgltf::sources::URI>(
					primitiveData.baseTexture.value()).uri.c_str();

			auto [result, Id] = getTextureIdIfExistsInCache(
				TextureCaches[registeredId].texturesLoaded,
				primitiveData.baseColorTextureIndex.value());
			if (result)
			{
			...
			else
			{
				...
				TextureCaches[registeredId].texturesLoaded[
					primitiveData.baseColorTextureIndex.value()] = textureId;
			}
		}
		else if (std::holds_alternative<fastgltf::sources::BufferView>(
			primitiveData.baseTexture.value()))
		{
			const auto& bufferViewSource = std::get<fastgltf::sources::BufferView>(
				primitiveData.baseTexture.value());
