...
namespace clz::physics
{
	...
	void refreshAttachedShapes(const RigidBodyId rigidBodyId)
	{
		...
		{
			CLZ_ASSERT(
				B3_ID_EQUALS(Bodies[rigidBodyId], shapes[i].getAttachedBodyId()),
	...
	/// @copydoc
	void
	setBodyLinearLocks(const RigidBodyId rigidBodyId, const std::array<bool, 3>& linearLocks)
	{
		const auto& bodyId = Bodies[rigidBodyId];
	...
	/// @copydoc
	void
	setBodyAngularLocks(const RigidBodyId rigidBodyId, const std::array<bool, 3>& angularLocks)
	{
		const auto& bodyId = Bodies[rigidBodyId];
		b3MotionLocks current = b3Body_GetMotionLocks(bodyId);
