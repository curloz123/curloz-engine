...
template <typename T>
void disableComponent(const entity e)
{
	auto& componentStorage = getStorage<T>();
	if (componentStorage.sparse[e] == NULL_STORAGE_INDEX)
	{
		log::warn("Attempt to disable non existing component"
				"for entity: " + std::to_string(static_cast<uint32_t>(e)));
		return;
	}
...
template <typename T>
void enableComponent(const entity e)
{
	auto& componentStorage = getStorage<T>();
	if (componentStorage.sparse[e] == NULL_STORAGE_INDEX)
	{
		log::warn("Attempt to enable non existing component"
				"for entity: " + std::to_string(static_cast<uint32_t>(e)));
		return;
	}

	componentStorage.dense.emplace_back(e);
	std::sort(componentStorage.dense.begin(), componentStorage.dense.end(),
