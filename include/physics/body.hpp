...
namespace clz::physics
{
	...
	enum class BodyType
	{
		...
		KinematicBody = b3_kinematicBody, ///< @brief Body moved manually via code (ignores forces).
	...
	struct BodyData
	{
		BodyType type = BodyType::DynamicBody;			  ///< @brief The type of the body.
		bool enableSleep = true;				  ///< @brief Whether the body is allowed to sleep when at rest.
		math::vec3 position = math::vec3(0.0f, 0.0f, 0.0f);	  ///< @brief Initial world-space position.
		math::quat rotation = math::quat(1.0f, 0.0f, 0.0f, 0.0f); ///< @brief Initial world-space rotation.
		float linearDamping = 0.0f;				  ///< @brief Linear damping coefficient.
		float angularDamping = 0.1f;				  ///< @brief Angular damping coefficient.
		std::array<bool, 3> linearLocks = {false, false, false};  ///< @brief Locks for linear motion on X, Y, Z axes.
		std::array<bool, 3> angularLocks = {false, false, false}; ///< @brief Locks for angular motion on X, Y, Z axes.
		std::vector<ShapeDef> ShapeDefs = {};			  ///< @brief List of shape definitions to attach upon creation.
	};
} // namespace clz::physics
