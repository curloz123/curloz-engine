...
class Shape
{
	...
	bool needsRecreation = false; ///< @brief Flag indicating if shape properties changed
				      ///< and require rebuilding.
	bool shouldBeDestroyed = false; ///< @brief Flag indicating if the shape is marked
	...
	/// @param isRecreating If true, marks the shape for recreation rather than permanent
	/// deletion.
	...
	{
		CLZ_ASSERT(
			B3_IS_NON_NULL(otherShape.getShapeId()) && B3_IS_NON_NULL(m_shapeId),
	...
	{
		CLZ_ASSERT(
			B3_IS_NON_NULL(m_shapeId) && B3_IS_NON_NULL(otherShape.getShapeId()),
