...
namespace clz::renderer
{
	...
	struct PrimitiveData
	{
		...
		/**
		 * @brief Optional source for the base color texture (either a file URI or an embedded buffer view).
		 */
	...
	/**
	 * @struct Primitive
	 * @brief Represents a single drawable primitive with GPU-side buffer offsets and texture bindings.
	 */
	struct Primitive
	{
		uint32_t indexCount;			/**< @brief Number of indices to draw. */
		uint32_t firstIndex;			/**< @brief Offset to the first index in the global index buffer. */
		uint32_t baseVertexIndex;		/**< @brief Offset to the first vertex in the global vertex buffer. */
		std::optional<TextureID> baseTextureId; /**< @brief ID of the bound base color texture, if any. */
	};
	...
	struct Mesh
	{
		std::vector<Primitive> primitives; /**< @brief List of primitives contained in this mesh. */
	};
	...
	struct Node
	{
		std::optional<uint32_t> meshIndex; /**< @brief Index of the mesh associated with this node, if any. */
		math::mat4 localTransform;	   /**< @brief Local transformation matrix relative to its parent. */
	...
	struct Model
	{
		std::vector<std::optional<Mesh>> meshes; /**< @brief List of meshes (optional to handle sparse indices). */
		 ...
		 * @param commandBuffer The Vulkan command buffer to record commands into.
		 */
		void drawNode(const auto& node, const math::mat4& parentTransform, VkCommandBuffer commandBuffer);
		 ...
		 */
		Node loadNode(const fastgltf::Node& node, const fastgltf::Asset& asset, ModelId registeredId);
		 ...
		 */
		Mesh loadMesh(const fastgltf::Mesh& mesh, const fastgltf::Asset& asset, ModelId registeredId);

		/**
		 * @brief Registers primitive data to global GPU buffers and handles texture loading/caching.
		 * @param primitiveData The raw CPU-side primitive data.
		 * @param asset The parent fastgltf asset.
		 * @param registeredId The ID of the model being loaded.
		 * @return The GPU-ready Primitive structure.
		 */
		Primitive registerPrimitive(const PrimitiveData& primitiveData, const fastgltf::Asset& asset, ModelId registeredId);
	};
	...
	/**
	 * @struct TextureCache
	 * @brief Caches loaded textures for a specific model to prevent redundant loading of shared textures.
	 */
	struct TextureCache
	{
		std::unordered_map<size_t, TextureID> texturesLoaded; /**< @brief Map of glTF texture index to internal TextureID. */
...
namespace clz::renderer
{
	 ...
	 * @param commandBuffer The Vulkan command buffer to record draw commands into.
	 */
	void drawModel(ModelId modelId, const math::vec3& position, const math::quat& rotation, const math::vec3& scale,
