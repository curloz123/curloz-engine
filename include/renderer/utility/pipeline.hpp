...
namespace clz::renderer
{
	...
	bool createPipelineLayout(PipelineContext& rPipelineContext, uint32_t pushConstantSize = 0,
	...
	VkPipelineDepthStencilStateCreateInfo createDepthStencilState(
		bool depthEnable = true, bool stencilEnable = false);

	VkPipelineRenderingCreateInfo createPipelineRenderingInfo(uint32_t attachmentCount,
		const std::vector<VkFormat>& attachmentFormats, VkFormat depthFormat);
