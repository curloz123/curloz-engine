...
namespace clz::editor::backend
{
	bool createEditorDescriptors()
	{
		...
		UBOPoolSize.descriptorCount = renderer::r_FRAMES_IN_FLIGHT * NUM_BACKEND_UNIFORM_BUFFERS;
		...
		renderer::setHandleName(reinterpret_cast<uint64_t>(descriptorPool),
			VK_OBJECT_TYPE_DESCRIPTOR_POOL, "editor descriptor pool");
		...
		// --- Allocate camera UBO descriptor sets, one per frame-in-flight ---
		{
			...
			const std::vector editorDescriptorLayouts(renderer::r_FRAMES_IN_FLIGHT, renderer::cameraUBOLayout);
			allocInfo.pSetLayouts = editorDescriptorLayouts.data();

			cameraDescriptorSets.resize(renderer::r_FRAMES_IN_FLIGHT);
			if (vkAllocateDescriptorSets(renderer::r_deviceContext.device, &allocInfo,
			...
			{
				renderer::setHandleName(reinterpret_cast<uint64_t>(cameraDescriptorSets[i]),
					VK_OBJECT_TYPE_DESCRIPTOR_SET, ("editor camera descriptor set " + std::to_string(i)).c_str());
			}
		}
		...
		for (uint32_t i = 0; i < renderer::r_FRAMES_IN_FLIGHT; ++i)
		{
			...
			vkUpdateDescriptorSets(renderer::r_deviceContext.device,
				1, &write, 0, nullptr);
		}
