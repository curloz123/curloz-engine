...
namespace clz::timemachine
{
	...
	inline void timeTravel()
	{
		...
		const bool leftShiftPressed =
			window::isKeyPressed(input::Key::LeftShift);
		const bool leftControlPressed =
			window::isKeyPressed(input::Key::LeftControl);
